// Fire (revised): fuel catches at the engine — in the sandbox, a couple of seconds after the first flare
// goes out — smoulders briefly, then burns. It doesn't spread. The longer it burns, the likelier the car
...
// The sandbox's fire test: arm it, and it catches a moment after the first flare burns out.
export function armFire(state) { state.hazards.fireArmed = true; }
...
export function stepFire(state, dt) {
  const hz = state.hazards;
  if (hz.fireArmed && !hz.fire && !state.flares.some((f) => f.state !== 'out')) { hz.fireArmed = false; spawnFire(state); }
  const f = hz.fire;
  if (!f) return;
  const fc = state.cfg.hazards.fire;
  f.t += dt;
